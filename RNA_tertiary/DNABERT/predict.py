...
#N = input('Indicate smaple size : "20", "60", "140", "240", "14102" : ')
...
for N in ["20", "60", "140", "240", "all"]:
    ...
    if mode == 'logreg':
        ...
        chosen_model = joblib.load(f"./models/{N}/logreg_dnabert_{N}.joblib")
        outname = f'logreg{N}'
        ...
        chosen_model = joblib.load(f'./models/{N}/mlp_dnabert_{N}.joblib')
        outname = f'mlp{N}'
        ...
        chosen_model = joblib.load(f'./models/{N}/tr_dnabert_{N}.joblib')
        outname = f'tr{N}'
        ...
        chosen_model = joblib.load(f'./models/{N}/rf_dnabert_{N}.joblib')
        outname = f'rf{N}'
        model = 'Random Forest'
        ...
        chosen_model = joblib.load(f'./models/{N}/xgb_dnabert_{N}.joblib')
        outname = f'xgb{N}'
        model = 'XGBoost'
        ...
        chosen_model = joblib.load(f'./models/{N}/lda_dnabert_{N}.joblib')
        outname = f'lda{N}'
        ...
        chosen_model = joblib.load(f'./models/{N}/knn_dnabert_{N}.joblib')
        outname = f'knn{N}'
        ...
        chosen_model = joblib.load(f'./models/{N}/gnb_dnabert_{N}.joblib')
        outname = f'gnb{N}'
        model = 'Naive-Bayes'
    ...
    with open('../dataset/test_15%.json') as f: # Change test set directory
        rnas = json.load(f)
    ...
    for rna in tqdm(rnas):
        ...
        try:
            ...
            structure_3mer = np.load('../dataset/3mer_cmaps_contacts_'+rnaid+'_contacts.npz')['b'] # Change input directory (original contact maps)
