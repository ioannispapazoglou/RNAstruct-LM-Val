...
for N in ["20", "500", "2500", "5000", "8000", "14076"]:
    ...
    if mode == 'logreg':
        ...
        chosen_model = joblib.load(f"./logreg_rnabert_{N}.joblib")
        outname = f'logreg{N}'
        ...
        chosen_model = joblib.load(f'./mlp_rnabert_{N}.joblib')
        outname = f'mlp{N}'
        ...
        chosen_model = joblib.load(f'./tr_rnabert_{N}.joblib')
        outname = f'tr{N}'
        ...
        chosen_model = joblib.load(f'./rf_rnabert_{N}.joblib')
        outname = f'rf{N}'
        model = 'Random Forest'
        ...
        chosen_model = joblib.load(f'./xgb_rnabert_{N}.joblib')
        outname = f'xgb{N}'
        model = 'XGBoost'
        ...
        chosen_model = joblib.load(f'./lda_rnabert_{N}.joblib')
        outname = f'lda{N}'
        ...
        chosen_model = joblib.load(f'./knn_rnabert_{N}.joblib')
        outname = f'knn{N}'
        ...
        chosen_model = joblib.load(f'./gnb_rnabert_{N}.joblib')
        outname = f'gnb{N}'
        model = 'Naive-Bayes'
    ...
    for rna in tqdm(rnas):
        ...
        structure_1mer = torch.tensor(dot_bracket_to_matrix(structure), dtype=torch.float32)
